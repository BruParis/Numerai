...
def single_cl_dt_proba(strat, strat_dir, model_dict, model_types, data_t, eras_type_df, cl):
    ...
    for era_b in eras_batches:
        ...
        proba_batches = pd.concat([proba_batches, pred_cl], axis=0)

    return proba_batches
...
def single_cluster_pred(strat, strat_dir, model_dict, model_types, data_types_fp,
    ...
    for data_t, fpath in data_types_fp:
        cl_proba = single_cl_dt_proba(
            strat, strat_dir, model_dict, model_types, data_t, eras_type_df, cl)
...
def make_prediction_fst(strat, strat_dir, model_dict, data_types_fp, eras_type_df, model_types):
    ...
    for data_t, fpath in data_types_fp:
        ...
        for eModel in model_types:
            full_cl_model_pred = aggregate_model_cl_preds(
                eModel, cl_dict, proba_dict)
            full_pred_df = pd.concat(
                [full_pred_df, full_cl_model_pred], axis=1)

        print("full_pred_df: ", full_pred_df)
        full_rank_df = rank_proba(full_pred_df, model_types)
        ...
        if data_t == VALID_TYPE:
            ...
            models_valid_score(final_pred_dict, model_types, full_rank_df)
