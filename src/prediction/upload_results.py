...
def load_predict_file(pred_fp, model_type):
    ...
    predict_df = pd.DataFrame(
        {'id': predict_series.index, TARGET_LABEL: predict_series.values}).set_index('id')
...
def upload_results(strat_dir):
    ...
    pred_test_fp = strat_dir + '/final_predict_test_' + pred_suffix + '.csv'
    pred_live_fp = strat_dir + '/final_predict_live_' + pred_suffix + '.csv'
