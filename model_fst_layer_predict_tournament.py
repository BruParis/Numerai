...
def make_pred_era(data_subsets_dirname, subsets, era, input_data):
    print("make_pred_era: ", era, " - input: ", input_data)

    if input_data.empty:
        return None

    era_data_corr = input_data.corr()

    era_sub_proba = {name: make_era_sub_proba(data_subsets_dirname, era, input_data, era_data_corr, name, subset)
...
def make_pred(data_subsets_dirname, subsets, input_data, eras_batch):
    ...
    input_batch = [input_data.loc[input_data['era'] == era].drop(
        'target_kazutsugi', axis=1) for era in eras_batch]
    eras_input_batch = zip(eras_batch, input_batch)

    bMultiProcess = False
    era_batch_pred_class = []
    if bMultiProcess:
        pred_era_param = list(zip(itertools.repeat(data_subsets_dirname), itertools.repeat(
            subsets), eras_input_batch))
        era_batch_pred_class = pool_map(make_pred_era, pred_era_param, collect=True)
    else:
        era_batch_pred_class = [make_pred_era(data_subsets_dirname, subsets, era, input_data) for era, input_data in eras_input_batch]
